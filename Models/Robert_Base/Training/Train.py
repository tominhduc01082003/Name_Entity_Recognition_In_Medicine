...
from Config_Hyper import (
    MODEL_NAME, MAX_LEN, BATCH_SIZE, NUM_EPOCHS, LR,
    WEIGHT_DECAY, WARMUP_RATIO, GRAD_ACCUM_STEPS, NUM_WORKERS,
    FP16, MAX_GRAD_NORM, SAVE_TOTAL_LIMIT, LOGGING_STEPS, METRIC_FOR_BEST_MODEL, OUTPUT_DIR
)
...
def main():
    set_seed(42)
    logger.info(f"Bắt đầu train vs model: {MODEL_NAME}")
    ...
    def compute_metrics(eval_pred: EvalPrediction):
        ...
        for p_row, l_row in zip(preds, labels):
            ...
            for p_id, l_id in zip(p_row, l_row):
                if l_id == -100:
                    continue
                p_tags.append(id2label[int(p_id)])
                g_tags.append(id2label[int(l_id)])
    ...
    if checkpoint_dirs:
        ...
        if os.path.exists(trainer_state_file):
            ...
            logger.warning("trainer_state.json không tồn tại, dùng log_history trong Trainer")
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=list(range(num_labels)))
