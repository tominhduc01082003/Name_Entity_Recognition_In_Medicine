...
from Config_Hyper_Vihealth import OUTPUT_DIR
...
def main():
    ...
    print("\nKết quả NER theo từng câu (sau khi tách từ bằng VnCoreNLP):\n")

    for sent_idx, sentence in enumerate(sentences, 1):
        word_segmented = rdrsegmenter.tokenize(sentence)
        segmented_sentence = " ".join(word_segmented[0]) 

        print(f"\n──────────────────────────────")
        print(f"Câu {sent_idx} gốc: {sentence}")
        print(f"Câu sau khi tách từ: {segmented_sentence}")
        encoding = tokenizer(segmented_sentence, return_tensors="pt", truncation=True)
        encoding = {k: v.to(device) for k, v in encoding.items()}
        tokens = tokenizer.convert_ids_to_tokens(encoding["input_ids"][0])[1:-1]

        with torch.no_grad():
            outputs = model(**encoding)
            predictions = torch.argmax(outputs.logits, dim=-1)[0].detach().cpu().tolist()[1:-1]

        print(f"\n  - > Tokenized ({len(tokens)} tokens): {tokens}")
